...
class TestSandboxAgentWorkflowConfig:
    ...
    def test_default_sandbox_config(self):
        ...
        assert config.sandbox_config["image"] == "nat-sandbox:latest"
        assert config.sandbox_config["memory_limit"] == "4g"
        assert config.sandbox_config["cpu_limit"] == 4.0
    ...
    def test_custom_sandbox_config(self):
        ...
        assert config.sandbox_config["memory_limit"] == "8g"

    def test_enabled_tools_filtering(self):
        """Test that enabled_tools can be specified."""
        config = SandboxAgentWorkflowConfig(
            llm_name="test_llm",
            enabled_tools=["shell", "python", "browser"],
        )

        assert config.enabled_tools == ["shell", "python", "browser"]
