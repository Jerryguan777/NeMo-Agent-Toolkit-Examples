...
class SandboxToolExecutor:
    ...
    All tools share the same sandbox instance, ensuring consistent
    state across operations (persistent shell, IPython kernel, browser).
    """
    ...
    def truncate(self, text: str) -> str:
        """Truncate output to maximum length (head + tail preserved)."""
        return truncate_output(text, self.max_output_chars)

    async def list_generated_files(self) -> list[str]:
        """List files in the output directory via persistent shell."""
        try:
            result = await self.sandbox.run_command(
                "ls -1 /workspace/output 2>/dev/null",
                timeout=10,
            )
            if result.success:
